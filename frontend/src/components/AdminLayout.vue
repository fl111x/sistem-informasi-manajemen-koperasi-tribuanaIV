  ...
  <div class="flex h-screen w-full bg-slate-50 font-sans text-slate-800 overflow-hidden">
    ...
    <aside class="w-64 bg-white border-r border-slate-200 flex flex-col justify-between flex-shrink-0 z-20">
      <div>
        ...
        <!-- Daftar Menu Dinamis Menggunakan router-link -->
        <nav class="p-3 flex flex-col gap-1">
          
          <router-link to="/" 
            class="flex items-center gap-3 px-3 py-2.5 text-sm rounded-lg transition-colors"
            :class="route.path === '/' ? 'bg-blue-600 text-white font-bold shadow-sm' : 'text-slate-600 font-medium hover:bg-slate-100'">
          ...
          <router-link v-if="user?.nama_role === 'Admin Sistem'" to="/kelola-voucher" 
            class="flex items-center gap-3 px-3 py-2.5 text-sm rounded-lg transition-colors"
            :class="route.path === '/kelola-voucher' ? 'bg-blue-600 text-white font-bold shadow-sm' : 'text-slate-600 font-medium hover:bg-slate-100'">
          ...
          <router-link v-if="['Admin Sistem', 'Admin Pembelian', 'Admin Order', 'Admin Gudang', 'Admin Penjualan'].includes(user?.nama_role)" to="/pembelian" 
            ...
            {{ user?.nama_role === 'Admin Order' ? 'List Order Barang' : 'Gudang & Pembelian' }}
          </router-link>
