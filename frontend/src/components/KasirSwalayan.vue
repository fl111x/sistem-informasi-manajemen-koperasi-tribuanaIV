...
// DATA DARI DATABASE
// ==========================================
const masterBarangSwalayan = ref([]);
...
const hasilPencarian = computed(() => {
  ...
  return masterBarangSwalayan.value.filter(item => 
...
const tambahKeFaktur = (item) => {
  if (Number(item.harga_swalayan) <= 0) {
    return tampilkanNotif('Gagal', 'Harga barang ini belum diset. Silakan hubungi admin.');
  }
          ...
          <div v-if="searchQuery.length > 0" class="absolute w-full top-full left-0 bg-white border-b border-x border-slate-300 shadow-lg max-h-60 overflow-y-auto rounded-b-md">
            <ul v-if="hasilPencarian.length > 0">
              ...
              >
                ...
                <span v-if="Number(item.harga_swalayan) > 0" class="text-sm font-bold text-slate-600">{{ formatRupiah(item.harga_swalayan) }}</span>
      ...
      <div class="w-[380px] flex flex-col h-full bg-white border border-slate-300 rounded-lg shadow-sm p-4 justify-between">
        ...
        <div class="flex flex-col gap-3">
          ...
          <div>
            ...
            <div v-if="selectedAnggotaObj" class="mt-1 flex justify-between items-center text-xs">
              <span class="text-slate-500">Saldo Voucher:</span>
              <span class="font-bold text-indigo-600">{{ formatRupiah(selectedAnggotaObj.saldo_voucher) }}</span>
            </div>
          </div>
